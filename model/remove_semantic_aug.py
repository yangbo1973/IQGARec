...
class MyModel6(torch.nn.Module):
    def __init__(self, hidden_size, item_num, state_size, emb_dropout_rate, dropout_rate, delta, 
                 lam_history, lam_intent, lam_vqloss, lam_cl_loss, ratio_substitute,
        ...
        self.item_embeddings = torch.nn.Embedding(self.item_num + 2, self.hidden_size, padding_idx = 0)
        ...
        # Vector Quantizer
        self.vq = VectorQuantizer(self.num_clusters, self.hidden_size, self.dropout_rate).to(self.device)
        self.init_vq()
    ...
    def init_vq(self):
        ...
        # 执行KMeans聚类
        kmeans = KMeans(n_clusters=self.num_clusters, random_state=0, n_init='auto').fit(item_emb_weight_np)
        # 返回聚类中心（PyTorch Tensor）
        centers = torch.tensor(kmeans.cluster_centers_, 
                          dtype=self.item_embeddings.weight.dtype, 
                          device=self.device)
        self.vq.embeddings.weight = torch.nn.Parameter(centers)
    ...
    def forward(self, seq, x_t, step, train_flag = False):
        ...
        quantized, vq_loss, indices = self.vq(item_emb, mask_seq) 
    ...
    def get_all_indices(self):
        ...
        _, _, indices = self.vq(emb_weight, mask)
        return indices

    
    def random_augmentation(self, seq, mask_ratio=0.2, crop_ratio=0.5, reorder_ratio=0.1):
        """
        对 batch 序列 seq: [batch, max_len] 逐条独立做数据增强。
        每条序列随机选择 mask / crop / reorder 中的一种。
        增强后序列长度可能与原长不同，统一补齐或截断到 self.state_size。
        """
        import random
        batch_size, max_len = seq.shape
        mask_id = self.item_num + 1
        aug_seqs = []

        for i in range(batch_size):
            # 取出第 i 条序列的有效部分（去 padding）
            mask_nonzero = seq[i] != 0
            valid_items = seq[i][mask_nonzero].tolist()
            seq_len = len(valid_items)

            if seq_len == 0:
                # 全 padding 的序列保持不变
                aug_seqs.append(seq[i].clone())
                continue

            # 等概率选择一种增强
            aug_type = random.choice(['mask', 'crop', 'reorder'])
            seq_aug = valid_items[:]  # python list

            if aug_type == 'mask':
                if mask_ratio > 0:
                    num_mask = max(1, int(seq_len * mask_ratio))
                    num_mask = min(num_mask, seq_len)
                    indices = random.sample(range(seq_len), num_mask)
                    for idx in indices:
                        seq_aug[idx] = mask_id

            elif aug_type == 'crop':
                keep_len = max(1, int(seq_len * crop_ratio))
                keep_len = min(keep_len, seq_len)
                start = random.randint(0, seq_len - keep_len)
                seq_aug = seq_aug[start:start + keep_len]

            elif aug_type == 'reorder':
                if reorder_ratio > 0 and seq_len >= 2:
                    reorder_len = max(2, int(seq_len * reorder_ratio))
                    reorder_len = min(reorder_len, seq_len)
                    start = random.randint(0, seq_len - reorder_len)
                    end = start + reorder_len
                    sub = seq_aug[start:end]
                    random.shuffle(sub)
                    seq_aug[start:end] = sub

            # 统一到固定长度 self.state_size，左 padding 0（与原数据处理方式一致）
            aug_len = len(seq_aug)
            if aug_len >= self.state_size:
                aug_seq_tensor = torch.tensor(seq_aug[-self.state_size:], dtype=seq.dtype, device=seq.device)
            else:
                pad_len = self.state_size - aug_len
                pad = torch.zeros(pad_len, dtype=seq.dtype, device=seq.device)
                aug_seq_tensor = torch.cat([pad, torch.tensor(seq_aug, dtype=seq.dtype, device=seq.device)])
            aug_seqs.append(aug_seq_tensor)

        return torch.stack(aug_seqs, dim=0)
    ...
    def augmentation(self, seq, ratio=0.1):
        ...
        if len(flat_items) > 0:
            ...
            _, _, item_indices = self.vq(item_embs, torch.ones(len(flat_items), device=self.device))
    ...
    def calculate_loss(self, seq, target):
        ...
        x_predicted, vq_loss = self.forward(seq, x_noise, t, train_flag=True)
        ...
        seq_aug = self.random_augmentation(seq)
        #seq_aug = seq.clone()
        ...
        loss = loss_diffu_value + self.lam_vqloss * vq_loss + self.lam_cl_loss * cl_loss
        return loss, cl_loss
